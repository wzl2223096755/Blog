...
(function() {
  ...
  const SUBJECTS = ['数据结构', '计算机组成原�?, '操作系统', '计算机网�?, '数学一', '英语一', '政治', '其他'];
  ...
  async function showBackupManager() {
    ...
    if (history.length === 0) {
      alert('暂无自动备份记录。系统会�?4小时自动创建一个备份�?);
      return;
    }
    ...
    modal.innerHTML = `
      <div style="background:var(--card);border-radius:8px;padding:24px;max-width:600px;max-height:80vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.2);">
        ...
        <p style="font-size:13px;color:var(--ink-soft);margin-bottom:20px;">系统�?4小时自动备份一次，最多保留最�?{MAX_AUTO_BACKUPS}个备份�?/p>
      ...
      return `
        <div style="border:1px solid var(--border);border-radius:4px;padding:12px;margin-bottom:10px;background:#fff;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            ...
            <button class="btn-primary" data-restore="${idx}" style="padding:6px 12px;font-size:12px;">恢复此备�?/button>
          </div>
          <div style="font-size:12px;color:var(--ink-soft);">
            学习�?${totalCards} �?· 习惯 ${totalHabits} �?· 计划 ${(backup.scheduleRows || []).length} �?          </div>
        </div>
      `;
    }).join('');
  ...
  function cardMeta(c, today) {
    if (isDue(c, today)) return { key: 'due', label: '待复�? };
    if (isNew(c)) return { key: 'new', label: '新学' };
    const ivl = c.ivl || 0;
    if (ivl >= 6) return { key: 'mastered', label: '已掌�? };
    if (ivl >= 1) return { key: 'learning', label: '学习�? };
    return { key: 'relearn', label: '重新学习' };
  }

  // 改进的间隔重复算法（基于SM-2简化版�?  function reviewCard(id, grade) {
    const c = feynmanEntries.find(e => e.id === id);
    if (!c) return;
    
    // 初始化难度系数（easeFactor�?    if (!c.easeFactor) c.easeFactor = 2.5;
    ...
    if (grade === 'again') {
      ...
      // 模糊：维持或稍微倒退，略微降低难度系�?      next = Math.max(0, idx - 1);
      newEaseFactor = Math.max(1.3, c.easeFactor - 0.15);
    } else if (grade === 'good') {
      // 记住：正常进�?      next = Math.min(idx + 1, INTERVALS.length - 1);
      // easeFactor不变
    } else {
      // 轻松：快速进步，提高难度系数
  ...
  // 快捷键处�?  function setupKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
      ...
      // Esc: 取消编辑或关闭打开的卡�?      if (e.key === 'Escape') {
        if (editingId) {
          cancelEdit();
        } else if (openCardId) {
          openCardId = null;
      ...
      // Ctrl/Cmd + F: 聚焦搜索�?      if ((e.ctrlKey || e.metaKey) && e.key === 'f') {
        e.preventDefault();
        document.querySelector('#search-input')?.focus();
      }
      
      // Ctrl/Cmd + B: 打开备份管理�?      if ((e.ctrlKey || e.metaKey) && e.key === 'b') {
        e.preventDefault();
        showBackupManager();
      }
      ...
      // 数字�?1-4: 快速评分（仅当有打开的卡片时�?      if (openCardId && !editingId && ['1','2','3','4'].includes(e.key)) {
  ...
  function showHelpPanel() {
    ...
    modal.innerHTML = `
      <div class="modal-content" style="max-width:800px;">
        ...
        <h3 class="modal-title">📖 使用帮助与技�?/h3>
        
        <div class="help-tabs">
          <button class="help-tab active" data-tab="quickstart">快速开�?/button>
          <button class="help-tab" data-tab="shortcuts">快捷�?/button>
          <button class="help-tab" data-tab="tips">使用技�?/button>
        ...
        <div class="help-content active" data-content="quickstart">
          <div class="help-section">
            ...
            <ol style="padding-left:20px;margin:0;">
              <li style="margin-bottom:8px;"><strong>选择科目</strong>：从下拉列表选择，或使用默认�?其他"</li>
              <li style="margin-bottom:8px;"><strong>填写主题</strong>：例�?操作系统的死�?</li>
              <li style="margin-bottom:8px;"><strong>用自己的话讲�?/strong>：假装给完全不懂的人解释，越通俗越好</li>
              <li style="margin-bottom:8px;"><strong>记录卡壳的地�?/strong>：哪里讲不清楚？这就是要补的地方</li>
              <li style="margin-bottom:8px;"><strong>简化总结</strong>：用一两句话或一个类比浓缩这个概�?/li>
            </ol>
            <div class="help-tip">
              💡 <strong>技�?/strong>：创建卡片时�?<span class="help-kbd">Ctrl</span> + <span class="help-kbd">S</span> 快速保�?            </div>
          </div>
          ...
          <div class="help-section">
            ...
            <ol style="padding-left:20px;margin:0;">
              <li style="margin-bottom:8px;">点击卡片标题�?strong>先不看内�?/strong></li>
              <li style="margin-bottom:8px;">尝试用自己的话默讲一�?/li>
              <li style="margin-bottom:8px;">展开卡片对照，根据记忆程度评分：
                <ul style="margin-top:6px;padding-left:20px;">
                  <li><strong>1/忘记</strong>：完全想不起�?�?重新学习</li>
                  <li><strong>2/模糊</strong>：记得一些但不完�?�?需要巩�?/li>
                  <li><strong>3/记住</strong>：基本都能讲出来 �?正常进步（最常用�?/li>
                  <li><strong>4/轻松</strong>：非常熟练，可以教别�?�?谨慎使用</li>
                </ul>
              </li>
            </ol>
            <div class="help-warning">
              ⚠️ <strong>注意</strong>：不要高估自己！�?轻松"会大幅延长复习间隔，可能导致遗忘。只在真的非常熟练时使用�?            </div>
          </div>
          ...
          <div class="help-section">
            ...
            <ul class="help-list">
              ...
              <li>优先复习标记�?待复�?的卡�?/li>
              <li>学习新内容后立即创建卡片</li>
              <li>每周查看统计面板�?span class="help-kbd">Ctrl</span>+<span class="help-kbd">I</span>）了解进�?/li>
              <li>定期导出备份到云盘保�?/li>
            </ul>
          </div>
        </div>
        ...
        <div class="help-content" data-content="tips">
          <div class="help-section">
            ...
            <div style="background:#f5f5f5;padding:12px;border-radius:4px;margin:8px 0;">
              <div style="font-size:13px;margin-bottom:4px;"><strong>主题</strong>：操作系�?/div>
            ...
            <div style="background:var(--sage-soft);padding:12px;border-radius:4px;margin:8px 0;">
              <div style="font-size:13px;margin-bottom:4px;"><strong>主题</strong>：操作系统中的死锁及其四个必要条�?/div>
              <div style="font-size:12px;"><strong>讲解</strong>：死锁就像四辆车在十字路口都不让路。四个条件：1）互斥（车不能重叠）2）占有且等待（占着路还要等�?）不可抢占（不能把别的车推走�?）循环等待（A等B，B等C，C等D，D等A�?/div>
            </div>
          </div>
          ...
          <div class="help-section">
            <h4>🎯 盲点清单使用技�?/h4>
            <ul class="help-list">
              <li><strong>具体描述</strong>�?死锁的第三个条件不理�? �?"不懂" 更好</li>
              <li><strong>及时攻克</strong>：标记盲点后，针对性学习，理解后立即勾�?/li>
              <li><strong>定期回顾</strong>：查看攻克进度（统计面板），保持进步�?/li>
              <li><strong>不要害�?/strong>：盲点越多说明你越诚实，这是进步的起�?/li>
            </ul>
          </div>
          
          <div class="help-section">
            <h4>📈 科目平衡策略</h4>
            <p>�?<span class="help-kbd">Ctrl</span>+<span class="help-kbd">I</span> 查看科目分布�?/p>
            <ul class="help-list">
              <li><strong>避免失衡</strong>：某科占比超�?0%要注�?/li>
              <li><strong>重点科目40-50%</strong>：给重要科目更多关注</li>
              <li><strong>其他科目平均</strong>：保持全面发�?/li>
              <li><strong>每周调整</strong>：根据考试时间动态调整比�?/li>
            </ul>
          </div>
          ...
          <div class="help-section">
            ...
            <div class="help-tip">
              ...
              🌙 <strong>睡前</strong>：记忆巩固，适合快速过一�?br>
              💡 <strong>碎片时间</strong>：通勤时可以默想卡片内�?            </div>
            <p><strong>每日建议�?/strong></p>
            <ul class="help-list">
              <li>待复�?�?10张：15-20分钟</li>
              <li>待复�?10-20张：20-30分钟</li>
              <li>待复�?> 20张：需要补课，先学习再复习</li>
            </ul>
          </div>
          ...
          <div class="help-section">
            <h4>🔍 搜索技�?/h4>
            <ul class="help-list">
              <li>搜索关键词会高亮显示，方便定�?/li>
              <li>搜索 + 科目筛选组合使用更精准</li>
              <li>搜索"死锁"比搜�?操作系统 死锁"范围更广</li>
        ...
        <div class="help-content" data-content="faq">
          <div class="help-section">
            ...
            <div class="help-tip">
              💾 <strong>保护数据</strong>：定期点�?导出备份"保存到云盘（如OneDrive、百度网盘）
            </div>
          </div>
          
          <div class="help-section">
            <h4>�?自动备份在哪里？</h4>
            <p>自动备份也存在浏览器本地存储中，不是独立文件。按 <span class="help-kbd">Ctrl</span>+<span class="help-kbd">B</span> 可以查看和恢复最�?个自动备份�?/p>
            <div class="help-warning">
              ⚠️ 清除浏览器数据会<strong>同时删除</strong>自动备份！务必手动导出备份文件�?            </div>
          </div>
          ...
          <div class="help-section">
            ...
            <ol style="padding-left:20px;margin:0;">
              <li style="margin-bottom:6px;">在设备A�?导出备份"</li>
              <li style="margin-bottom:6px;">将备份文件传到设备B（邮件、聊天工具、云盘等�?/li>
              <li style="margin-bottom:6px;">在设备B�?导入备份"</li>
            </ol>
          </div>
          
          <div class="help-section">
            <h4>�?为什么我�?轻松"后很久才复习�?/h4>
            <p>这是<strong>正常�?/strong>！系统使用改进的SM-2算法�?/p>
            <ul class="help-list">
              <li>�?轻松"会提高难度系数，延长复习间隔</li>
              <li>说明你真的掌握了，不需要频繁复�?/li>
              <li>如果担心遗忘，下次�?记住"而不�?轻松"</li>
            </ul>
          </div>
          ...
          <div class="help-section">
            ...
            <p>几个应对策略�?/p>
            <ul class="help-list">
              <li><strong>减少新增</strong>：暂停创建新卡片，专注复�?/li>
              <li><strong>提高标准</strong>：不是所有内容都值得做卡�?/li>
              <li><strong>归档旧卡</strong>：考完的科目可以导出后删除</li>
              <li><strong>合并相似</strong>：多张讲同一概念的卡片可以合�?/li>
            </ul>
          </div>
  ...
  function calculateStats() {
    ...
    // 最�?天新�?    const recentCards = feynmanEntries.filter(e => {
      if (!e.date) return false;
      return e.date >= fmtDate(weekAgo);
    }).length;
    
    // 最�?天复习次�?    const recentReviews = feynmanEntries.reduce((sum, e) => {
      ...
      <div class="modal-content">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            ...
            <div class="stat-label">�?天新�?/div>
          </div>
          <div class="stat-card">
            <div class="stat-value">${stats.recentReviews}</div>
            <div class="stat-label">�?天复�?/div>
          </div>
        </div>
        ...
        <div style="margin-top:20px;">
          ...
          <div style="font-size:13px;color:var(--ink-soft);margin-bottom:8px;">
            已攻�?${stats.solvedGaps} / ${stats.totalGaps} 个盲�?          </div>
        ...
        <div style="margin-top:20px;">
          <h4 style="font-size:15px;margin:0 0 10px;font-family:'Newsreader',serif;">习惯完成率（�?天）</h4>
          <div style="font-size:13px;color:var(--ink-soft);margin-bottom:8px;">
  ...
  function render() {
    ...
    app.innerHTML = `
      <div class="stats-line">
        学习�?<b>${feynmanEntries.length}</b> �?· 习惯 <b>${habits.length}</b> �?· 计划 <b>${scheduleRows.length}</b> �?        <button class="btn-text" id="show-stats-btn" style="margin-left:12px;padding:4px 8px;text-decoration:none;border:1px solid var(--border);border-radius:3px;font-size:11px;">📊 统计</button>
        ...
        <span style="float:right;font-size:10px;color:var(--ink-soft);">
          快捷�? Ctrl+N 新建 · Ctrl+S 保存 · Ctrl+F 搜索 · Ctrl+H 帮助
        </span>
      </div>
      ...
      <div class="welcome-card">
        ...
        <p style="margin:0 0 16px;font-size:13.5px;line-height:1.65;">
          这是一个基�?strong>费曼学习�?/strong>�?strong>间隔重复算法</strong>的学习工具。让我们�?分钟快速上手：
        </p>
        <div class="welcome-steps">
          <div class="welcome-step">
            ...
            <div class="welcome-step-content">
              ...
              <div class="welcome-step-desc">�?Ctrl+N 或在下方表单填写。用自己的话讲解概念，记录不理解的地方�?/div>
            </div>
          </div>
          <div class="welcome-step">
          ...
          <div class="welcome-step">
            ...
            <div class="welcome-step-content">
              ...
              <div class="welcome-step-desc">�?Ctrl+F 搜索卡片，Ctrl+I 查看统计，Ctrl+H 查看完整帮助�?/div>
            </div>
          </div>
        </div>
        <div class="welcome-actions">
          <button class="btn-primary" id="dismiss-welcome">开始使�?/button>
      ...
      <div class="layout">
        <section class="panel" id="feynman-panel">
          <div class="panel-title">费曼学习�?<span class="due-badge">待复�?${dueCount}</span>${newCount > 0 ? ` <span class="new-badge">新学 ${newCount}</span>` : ''}</div>
          ...
          <div style="margin-bottom:14px;">
            ...
            ${searchQuery ? `<div style="font-size:11.5px;color:var(--blue);margin-top:4px;">找到 ${searchFiltered.length} 张卡�?/div>` : ''}
          </div>
          ...
          <div class="new-card-form">
            ...
            <div class="subject-row">
              ...
              <input type="text" id="f-chapter" placeholder="章节 / 标签（可选），如：死�? value="${editing ? escapeAttr(editing.chapter) : ''}">
            </div>
            <label class="field-label">�?概念 / 主题
              <span class="tip-badge" title="要具体！不要�?操作系统'，要�?操作系统的死�?">💡</span>
            </label>
            ...
            <textarea id="f-explain" placeholder="假装对方零基础，尽量用类比、不用术�? maxlength="2000">${editing ? escapeHtml(editing.explain) : ''}</textarea>
            <label class="field-label">�?哪里讲不清楚 / 卡壳�?              <span class="tip-badge" title="诚实记录！这些会进入盲点清单">💡</span>
            </label>
            <textarea id="f-gaps" placeholder="写下卡住的地方，这就是真正要回去补的地方" maxlength="1000">${editing ? escapeHtml(editing.gaps) : ''}</textarea>
            <label class="field-label">�?回去补完后，再简化一�?              <span class="tip-badge" title="用一两句话浓缩核�?>💡</span>
            </label>
          ...
          <div id="fcard-list">
            ${sortedEntries.length === 0 ? `
              <div class="empty-state">
                ${searchQuery ? '没有找到匹配的卡�? : `
                  <div style="padding:20px;">
                    <div style="font-size:48px;margin-bottom:12px;">📚</div>
                    <div style="font-size:15px;margin-bottom:12px;color:var(--ink);">还没有学习卡�?/div>
                    <div style="font-size:13px;line-height:1.6;margin-bottom:16px;">
                      学完一个概念就创建一张卡�?br>
                      用自己的话讲一遍，找出不理解的地方
                    </div>
                    <button class="btn-primary" onclick="document.querySelector('#f-topic').focus();document.querySelector('#feynman-panel').scrollIntoView({behavior:'smooth'});">创建第一张卡�?/button>
                  </div>
                `}
              </div>
            ` :
              sortedEntries.map(e => {
                ...
                <div class="fcard ${isOpen ? 'open':''} ${meta.key==='due' ? 'due':''} ${editingId===e.id ? 'editing':''}" data-id="${e.id}">
                  ...
                  <div class="fcard-body">
                    <div class="fcard-step"><span class="step-label">讲给别人�?/span>${query ? highlightText(e.explain||'�?, query) : (escapeHtml(e.explain)||'�?)}</div>
                    <div class="fcard-step"><span class="step-label">卡壳的地�?/span>${query ? highlightText(e.gaps||'�?, query) : (escapeHtml(e.gaps)||'�?)}</div>
                    <div class="fcard-step"><span class="step-label">简化后</span>${query ? highlightText(e.simplify||'�?, query) : (escapeHtml(e.simplify)||'�?)}</div>
                    <div class="review-row">
                      <span class="step-label">复习评分（决定下次复习时间）${isOpen ? ' �?或按数字�?1-4' : ''}</span>
            ...
            ` :
              gapItems.map(e => `
                <div class="gap-item ${e.gapDone ? 'done':''}">
                  <span class="gap-check" data-gap="${e.id}" title="${e.gapDone ? '撤销已攻�? : '标记已攻�?}">${e.gapDone ? '�? : '�?}</span>
                  <div class="gap-text">
        ...
        <section class="panel" id="habit-panel">
          ...
          <div class="habit-add-row">
            <input type="text" id="h-name" placeholder="添加一个习�?>
            <button class="btn-primary" id="h-add">添加</button>
          </div>
          <div class="week-nav">
            <div><button id="week-prev">�?上一�?/button></div>
            <span>${weekLabel}</span>
            <div>
              ${isThisWeek ? '' : '<button id="week-today" class="btn-today">回到本周</button>'}
              <button id="week-next">下一�?�?/button>
            </div>
          </div>
          ${habits.length === 0 ? `
            <div class="empty-state">
              <div style="padding:20px;">
                ...
                <div style="font-size:13px;color:var(--ink-soft);margin-bottom:12px;">还没有习惯记�?/div>
                <div style="font-size:12px;color:var(--ink-soft);line-height:1.5;margin-bottom:12px;">
                  添加一个想要养成的习惯<br>
                  每天完成就打勾，积累连续天数
                </div>
                <div style="font-size:11px;color:var(--blue);">💡 建议�?-3个小习惯开�?/div>
              </div>
            </div>
          ` : `
          <table class="habit-table">
            ...
            <tbody>
              ${habits.map(h => `
                <tr>
                  <td class="habit-name-cell">${escapeHtml(h.name)}<button class="habit-row-remove" data-rmhabit="${h.id}" title="删除习惯">×</button>${currentStreak(h.id) > 0 ? `<div class="habit-streak">🔥 ${currentStreak(h.id)} �?/div>` : ''}</td>
                  ${dates.map(d => {
                    const ds = fmtDate(d);
      ...
      <section class="panel full-width" id="schedule-panel">
        <div class="panel-title">每周计划�?/div>
        <p class="panel-hint">自己加时间段、自己填每天安排，这是固定的每周模板，不跟具体日期绑�?/p>
        ...
        <table class="schedule-table">
          ...
          <tbody>
            ${scheduleRows.length === 0 ? `<tr><td colspan="9"><div class="empty-state">还没有时间段，点下面“添加时间段”开始排</div></td></tr>` :
              scheduleRows.map(r => `
                <tr>
                  <td><textarea class="slot-label-input" data-row-label="${r.id}" placeholder="�?9-11�?>${escapeHtml(r.label)}</textarea></td>
                  ${[0,1,2,3,4,5,6].map(di => `
                    <td><textarea class="cell-input" data-cell-row="${r.id}" data-cell-day="${di}">${escapeHtml(scheduleCells[r.id+'_'+di]||'')}</textarea></td>
                  `).join('')}
                  <td><button class="slot-row-remove" data-rmrow="${r.id}" title="删除这一�?>×</button></td>
                </tr>
              `).join('')
            }
          </tbody>
        ...
        <div class="add-slot-row"><button class="btn-primary" id="add-row-btn">添加时间�?/button></div>
      </section>
  ...
  function bindEvents(app) {
    ...
    if (saveBtn) saveBtn.onclick = () => {
      ...
      if (!topic.trim() && !explain.trim()) { alert('至少填「概�?主题」或「讲给别人听�?); return; }
      const id = editingId;
      editingId = null;
      if (id) {
    ...
    // 搜索框事�?    const searchInput = app.querySelector('#search-input');
    if (searchInput) {
      searchInput.oninput = () => {
    ...
    // 备份管理器按�?    const backupManagerBtn = app.querySelector('#backup-manager');
